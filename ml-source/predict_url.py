...
# Move load model to main block or lazy loading
model = None
feature_names = None
...
def analyze_webpage_content(url, timeout=5):
    ...
    try:
        ...
        # Abort if the server returns an error or blocks the request
...
def apply_heuristic_boost(base_prob, external_features, url_features):
    ...
    if external_features:
        ...
        if "Let's Encrypt" in ssl_issuer and age != -1 and age < 30:
            boost += 0.20
...
if __name__ == "__main__":
    ...
    deep_scan_mode = (mode_input == '2')
    mode_name = "Deep Scan 🔍" if deep_scan_mode else "Fast Mode ⚡"
    print(f"\n  Using: {mode_name}")
    ...
    while True:
        try:
            ...
            if not test_url:
                continue
            if test_url.lower() in ('quit', 'exit', 'q'):
            ...
            predict(test_url, deep_scan=deep_scan_mode)
